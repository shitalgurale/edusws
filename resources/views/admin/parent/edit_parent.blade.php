                            ...
                            <form method="POST" enctype="multipart/form-data" class="d-block ajaxForm" action="{{ route('admin.parent.update', ['id' => $user->id]) }}">
                               ...
                               <?php 
                                ...
                                <div class="fpb-7">
                                    ...
                                    <input type="text" class="form-control eForm-control inputDate" id="birthday" name="birthday" value="{{ isset($info->birthday) ? date('m/d/Y', $info->birthday) : '' }}" />
                                </div>
...
<!--KPDebug-->
                                       ...
                                       <select name="section_id" id="section_id" class="form-select eForm-select eChoice-multiple-with-remove"
                                            @if(isset($childs) && is_countable($childs) && count($childs) == 0) required @endif
                                         onchange="sectionWiseStudent(this.value)">
                                ...
                                <?php 
                                    $childs = DB::table('users')
                                        ...
                                        ->whereNotNull('name') // optional check to ensure valid data
                                        ->get();
                                    ?>
                                
                                @if(count($childs) > 0)
                                   @foreach($childs as $child)
                                        @if(empty($child->id) || empty($child->name))
                                            @continue
                                        @endif
                                        <div id="existing_row{{ $child->id }}">
                                            <div class="child_existing_row row">
                                                <div class="form-group col-xl-10 col-lg-5 col-md-5 col-sm-12 col-xs-12 pt-2">
                                                    <input type="text" name="student_name_existing[]" class="student_id form-control eForm-control" value="{{ $child->name }}" readonly>
                                                    <input type="hidden" name="student_id[]" value="{{ $child->id }}">
                                                </div>
                                                <div class="col-xl-1 col-lg-1 col-md-2 col-sm-12 mb-3 mb-lg-0 pt-2">
                                                    <button type="button" class="delete_child btn btn-icon btn-danger" onclick="removeExistingRow(this)"> <i class="bi bi-x"></i> </button>
                                                </div>
                                            </div>
                                        </div>
                                    @endforeach
                ...
                @php
                    $studentIds = DB::table('users')->where('parent_id', $user->id)->pluck('id');
                @endphp
                ...
                <div class="row mt-4">
                    <div class="col-md-12">
                        <div class="table-responsive">
                            <h5 class="mb-3">{{ get_phrase('Assigned Students Summary') }}</h5>
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    @foreach ($studentIds as $studentId)
                                        @php
                                            $details = (new \App\Http\Controllers\CommonController)->get_student_details_by_id($studentId);
                                        @endphp

                                        @if (empty($details) || empty($details->name) || empty($details->class_id))
                                            @continue
                                        @endif
