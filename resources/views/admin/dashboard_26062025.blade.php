...
{{-- TEMPORARY DEBUG OUTPUT --}}
	              ...
	              <div class="dashboard_ShortListItem">
	                ...
	                >
	                  ...
	                  <a href="{{ route('admin.student') }}" >
	                      ...
	                      <img
	                      src="{{ asset('assets/images/Green_Arrow.png') }}"
	                      alt=""
	                    />
	                   <!-- <svg
	                ...
	                >
	                  <div class="ds_item_details">
	                    <h4 class="dashboard-count">{{ DB::table('users')->where('role_id', 7)->where('school_id', auth()->user()->school_id)->get()->count() }}</h4>
	                    <p class="total_info">{{ get_phrase('Total Students') }}</p>
	                  </div>
	                  <div class="dashboard-icon">
	                   <!-- <img
	                      src="{{ asset('assets/images/Student_icon1.png') }}"
	                      alt=""
	                    />  -->
	                    <img
	                      src="{{ asset('assets/images/student_icon_resized_60x60.png') }}"
	                      alt=""
	                    />
	                  </div>
	                </div>
	              ...
	              <div class="dashboard_ShortListItem">
	                ...
	                >
	                  ...
	                  <a href="{{ route('admin.teacher') }}" >
	                      ...
	                      <img
	                      src="{{ asset('assets/images/Red_Arrow.png') }}"
	                      alt=""
	                    />
	                    <!--<svg
	                      xmlns="http://www.w3.org/2000/svg"
	                ...
	                >
	                  <div class="ds_item_details">
	                    <h4 class="dashboard-count">{{ DB::table('users')->where('role_id', 3)->where('school_id', auth()->user()->school_id)->get()->count() }}</h4>
	                    <p class="total_info">{{ get_phrase('Total Teachers') }}</p>
	                  </div>
	                  
	                  <div class="dashboard-icon">
	                   <!-- <img
	                      src="{{ asset('assets/images/Teacher_icon1.png') }}"
	                      alt=""
	                    />  -->
	                    <img
	                      src="{{ asset('assets/images/ChatGPT Image May 29, 2025, 04_56_26 PM (1).png') }}"
	                      alt=""
	                    />
	                  </div>
	                </div>
  ...
  <div class="dashboard_ShortListItem">
    <div class="dsHeader d-flex justify-content-between align-items-center">
      <h5 class="title">{{ get_phrase('Subscription Plan') }}</h5>
      <a href="{{ route('admin.subscription') }}">
        <img src="{{ asset('assets/images/Yellow_Arrow.png') }}" alt="" />
      </a>
    </div>
    ...
    <div class="dsBody d-flex justify-content-between align-items-center">
      <div class="ds_item_details">
        @php
          $current_package = DB::table('subscriptions')
              ->join('packages', 'subscriptions.package_id', '=', 'packages.id')
              ->where('subscriptions.school_id', auth()->user()->school_id)
              ->where('subscriptions.active', 1)
              ->select('packages.name', 'subscriptions.expire_date')
              ->orderByDesc('subscriptions.id')
              ->first();

          $expiry = $current_package && $current_package->expire_date
              ? date('d M Y', $current_package->expire_date)
              : 'N/A';
        @endphp

       <h4 class="dashboard-count"> {{ $current_package->name ?? 'N/A' }}</h4>
        <p class="total_info">{{ $expiry }}</p>
      </div>

      <div class="dashboard-icon">
        <img src="{{ asset('assets/images/Subscription1.png') }}" alt="" />
      </div>
    </div>
  </div>
</div>
	            ...
	            <!--<div class="col-md-3">
	              <div class="dashboard_ShortListItem">
	                <div
	                  class="dsHeader d-flex justify-content-between align-items-center"
	                >
	                  <h5 class="title">{{ get_phrase('Library') }}</h5>
	                  <a href="{{ route('admin.book.book_list') }}" >
	                ...
	                >
	                  <div class="ds_item_details">
	                    <h4 class="dashboard-count">{{ $total_books = DB::table('books') ->where('school_id', auth()->user()->school_id) ->sum('copies'); }}</h4>
	                    <p class="total_info">{{ get_phrase('Total Books') }}</p>
	                  </div>
	                  <div class="dashboard-icon">
	                    <img
	                      src="{{ asset('assets/images/open-book_resized_60x60.png') }}"
	                      alt=""
	                    />
	                  </div>
	                </div>
	            ...
	            <div class="col-md-3">
	              <div class="dashboard_ShortListItem">
	                ...
	                >
	                  <div class="ds_item_details">
	                    <h4 class="dashboard-count">{{ DB::table('users')->where('role_id', 3)->where('school_id', auth()->user()->school_id)->get()->count() }}</h4>
	                    <p class="total_info">{{ get_phrase('Total Fee Receipts') }}</p>
	                  </div>
	                  <div class="dashboard-icon">
	                    <img
	                      src="{{ asset('assets/images/fee_manager.png') }}"
	                      alt=""
	                    />
	                  </div>
	                </div>
	          ...
	          <div class="row">
	              ...
	              <div class="dashboard_ShortListItem">
	                ...
	                >
	                  ...
	                  <a href="{{ route('admin.parent') }}" ><!--class="ds_link ds_parent">-->
	                      <img
	                      src="{{ asset('assets/images/Blue-Arrow.png') }}" />
	                ...
	                >
	                  <div class="ds_item_details">
	                    <h4 class="dashboard-count">{{ DB::table('users')->where('role_id', 6)->where('school_id', auth()->user()->school_id)->get()->count() }}</h4>
	                    <p class="total_info">{{ get_phrase('Total Parents') }}</p>
	                  </div>
	                  <div class="ds_item_icon">
	                    <img
	                      src="{{ asset('assets/images/Parents_icon1.png') }}"
	                      alt=""
	                    />
	                  </div>
	                </div>
	            ...
	            <div class="col-md-3"> 
                  <div class="dashboard_ShortListItem">
                    ...
                    <div class="dsBody d-flex justify-content-between align-items-center">
                      ...
                      <div class="ds_item_icon">
                        <img
                          src="{{ asset('assets/images/Staff2.png') }}"
                          alt=""
                        />
                      </div>
                    </div>
	            ...
	            <div class="col-md-3">
	              <div class="dashboard_ShortListItem">
	                <div
	                  class="dsHeader d-flex justify-content-between align-items-center"
	                >
	                  <h5 class="title">{{ get_phrase('Drivers') }}</h5>
	                  <a href="{{ route('admin.driver.list') }}" >
	                      
	                      <img
	                      src="{{ asset('assets/images/Yellow_Arrow.png') }}"
	                      alt=""
	                    />
	                    <!--<svg
	                      xmlns="http://www.w3.org/2000/svg"
	                ...
	                >
	                  <div class="ds_item_details">
	                    <h4 class="dashboard-count">{{ DB::table('users')->where('role_id', 8)->where('school_id', auth()->user()->school_id)->get()->count() }}</h4>
	                    <p class="total_info">{{ get_phrase('Total Drivers') }}</p>
	                  </div>
	                  <div class="ds_item_icon">
	                    <img
	                      src="{{ asset('assets/images/Drivers1.png') }}"
	                      alt=""
	                    />
	                  </div>
	                </div>
	            ...
	            <div class="col-md-3"> 
                  <div class="dashboard_ShortListItem">
                    ...
                    <div class="dsBody d-flex justify-content-between align-items-center">
                      <div class="ds_item_details">
                        <h1 class="dashboard-count">
                          {{ $total_vehicles = DB::table('vehicles') ->where('school_id', auth()->user()->school_id) ->count(); }}
                        </h1>
                        <p class="total_info">{{ get_phrase('Total Vehicles') }}</p>
                      </div>
                      <div class="ds_item_icon">
                        <img
                          src="{{ asset('assets/images/Bus_icon.png') }}"
                          alt=""
                        />
                      </div>
                    </div>
