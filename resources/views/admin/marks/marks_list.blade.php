    ...
    <div class="col-12">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <div class="d-flex gap-2">
                <a href="{{ asset('assets/csv_file/marks_preview.csv') }}" class="btn btn-success btn-sm">
                    {{ get_phrase('Example CSV file') }} <i class="bi bi-download"></i>
                </a>

                <button type="button" class="btn btn-dark btn-sm bi bi-eye" 
                    onclick="largeModal('{{ route('admin.marks.csv_preview') }}', '{{ get_phrase('CSV Format Preview') }}');" 
                    data-toggle="tooltip" title="{{ get_phrase('Preview CSV format') }}">
                </button>
            </div>
            ...
            <div class="export position-relative">
                ...
                <ul class="dropdown-menu dropdown-menu-end eDropdown-menu-2">
                    <li>
                        <a class="dropdown-item" href="{{ route('admin.marks.list_pdf', [
...
<form action="{{ route('admin.marks.upload') }}" method="POST" enctype="multipart/form-data" class="mb-4">
    @csrf
    <input type="hidden" name="class_id" value="{{ $marks_data['class_id'] }}">
    <input type="hidden" name="section_id" value="{{ $marks_data['section_id'] }}">
    <input type="hidden" name="subject_id" value="{{ $marks_data['subject_id'] }}">
    <input type="hidden" name="session_id" value="{{ $marks_data['session_id'] }}">
    <input type="hidden" name="exam_category_id" value="{{ $marks_data['exam_category_id'] }}">

    <div class="row g-3 align-items-center">
        <div class="col-auto">
            <label for="csv_file" class="form-label">{{ get_phrase('Upload Marks CSV') }}</label>
        </div>
        <div class="col-auto">
            <input type="file" name="csv_file" id="csv_file" class="form-control" accept=".csv" required>
        </div>
        <div class="col-auto">
            <button type="submit" class="btn btn-primary">{{ get_phrase('Upload') }}</button>
        </div>
    </div>
</form>
