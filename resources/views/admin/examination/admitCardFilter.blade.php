...
<div class="row">
                    ...
                    <div class="att-filter d-flex flex-wrap">
                        ...
                        <div class="att-filter-option">
                            ...
                            <select name="section_id" id="section_id" class="form-select eForm-select eChoice-multiple-with-remove" required>
                                ...
                                <?php $sections = isset($page_data['class_id']) ? Section::where('class_id', $page_data['class_id'])->get() : collect(); ?>
                                <?php foreach($sections as $section): ?>
                                   <!-- <option value="{{ $section['id'] }}" {{ $page_data['section_id'] == $section['id'] ?  'selected':'' }}>{{ $section['name'] }}</option> -->
                                   <option value="{{ $section->id }}" {{ isset($page_data['section_id']) && $page_data['section_id'] == $section->id ? 'selected' : '' }}>
    ...
    {{ isset($page_data['session_id']) && $page_data['session_id'] == $session->id ? 'selected' : '' }}>
    {{ $session->session_title }}
            ...
            <div class="position-relative">
             ...
             <!--   <li>
                ...
                <li>
                  <button class="dropdown-item" onclick="Export()">PDF</button>
                </li>
              </ul>
            </div>
          </div>
                            ...
                            <div class="d-flex justify-content-between">
                                <table class="table table-borderless info-table">
                                    <tr>
                                        ...
                                        <td>{{$student_details->code ?? null }}</td>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{$student_details->name ?? null }}</td>
    ...
    <td>
        {{ isset($student_details->birthday) ? date('d M Y', $student_details->birthday) : 'N/A' }}
    </td>

    <td><strong>GENDER</strong></td>
    <td>{{ null_checker($student_details->gender ?? null) }}</td>
</tr>

<tr>
    <td><strong>PARENTS NAME</strong></td>
    <td>{{ null_checker($student_details->parent_name ?? null) }}</td>

    <td><strong>CONTACT</strong></td>
    <td>{{ null_checker($student_details->phone ?? null) }}</td>
</tr>

<tr>
    <td><strong>ADDRESS</strong></td>
    <td colspan="3">{{ null_checker($student_details->address ?? null) }}</td>
</tr>

<tr>
    <td><strong>SCHOOL NAME</strong></td>
    <td>{{ DB::table('schools')->where('id', auth()->user()->school_id)->value('title') ?? 'N/A' }}</td>

    <td><strong>EXAM CENTER</strong></td>
    <td>{{ $selected_admit_card->exam_center ?? 'N/A' }}</td>
</tr>
                                </table>
                              <div class="student-image ml-3">
    @if(!empty($student_details) && !empty($student_details->photo))
        <img src="{{ $student_details->photo }}" alt="Student Photo">
    @else
