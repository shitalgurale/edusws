            ...
            <tr>
                ...
                <td>
                    ...
                    @if($existingAttendance)
                        <input type="hidden" name="attendance_id[]" value="{{ $existingAttendance->id }}">
                    @endif

                    <div class="custom-control custom-radio">
                        <input type="radio" name="status-{{ $student_id }}" value="1" class="present" 
                            {{ $existingAttendance && $existingAttendance->status == 1 ? 'checked' : '' }} required>
                        {{ get_phrase('present') }} &nbsp;

                        <input type="radio" name="status-{{ $student_id }}" value="0" class="absent" 
                            {{ (!$existingAttendance || $existingAttendance->status == 0) ? 'checked' : '' }} required>
                        {{ get_phrase('absent') }}
                    </div>
                </td>
            </tr>
