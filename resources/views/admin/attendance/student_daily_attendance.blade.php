...
<div class="card-body attendance_content text-center" id="default_attendance_box">
  ...
  <br>
  <span id="attendance_message">{{ get_phrase('Search Attendance Report') }}</span>
</div>
</div></div>
@endsection
...
function getStudentList() {
    ...
    if (date && class_id && section_id) {
        $.ajax({
            url: '{{ route('admin.attendance.student_daily_attendance') }}',
            type: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
                date: date,
            ...
            success: function(response) {
                if ($.trim(response) === '') {
                    ...
                    $('#attendance_message').text("{{ get_phrase('No Attendance Found') }}");
                } else {
            ...
            error: function() {
                $('#student_content').hide();
                $('#default_attendance_box').show();
                $('#attendance_message').text("{{ get_phrase('Error fetching data') }}");
            }
        });
    } else {
        toastr.error("{{ get_phrase('Please select all fields.') }}");
    }
}
...
function download_csv() {
    ...
    form.method = 'POST';
    form.action = "{{ route('admin.exportStudentDailyCSV') }}";
    form.target = '_blank';
