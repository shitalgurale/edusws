        ...
        <tbody>
            @foreach ($promotion_list as $enrollment)
                ...
                @php
                    ...
                    if (!$student_details || empty($student_details->user_information)) continue;
                    ...
                    if (!$section_details || !$class_to_details || !$class_from_details) continue;
                @endphp
                ...
                <tr>
                    ...
                    <td>
                        <span class="eBadge ebg-success display-none-view" id="success_{{ $enrollment->id }}">Promoted</span>
                        <span class="eBadge ebg-primary" id="danger_{{ $enrollment->id }}">Not promoted yet</span>
                    </td>
                    <td>
                        <button type="button" class="btn btn-icon btn-success btn-sm" onclick="enrollStudent('{{ $enrollment->id . '-' . $class_id_to . '-' . $section_id_to . '-' . $session_id_to }}', '{{ $enrollment->id }}')">
                            {{ get_phrase('Enroll to') }} <strong>{{ $class_to_details->name }}</strong>
                        </button>
                        <br><br>
                        <button type="button" class="btn btn-icon btn-secondary btn-sm" onclick="enrollStudent('{{ $enrollment->id . '-' . $class_id_from . '-' . $section_id_from . '-' . $session_id_to }}', '{{ $enrollment->id }}')">
                            {{ get_phrase('Enroll to') }} <strong>{{ $class_from_details->name }}</strong>
                        </button>
                    </td>
                </tr>
            @endforeach
