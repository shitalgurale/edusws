					...
					<div class="row">
						<div class="col-md-6 pb-3">
							<div class="eForm-layouts">
					            ...
					            <!--<br />-->
                			    <!--<p class="column-title">{{ get_phrase('PACKAGE INFO') }}</p>     
                			    <div class="fpb-7">
                                    <label for="package_id" class="eForm-label">{{ get_phrase('Select Package') }}</label>
                                    <select name="package_id" id="package_id" class="form-select eForm-select">
                                        <option value="">{{ get_phrase('Loading packages...') }}</option>
                                    </select>
                                </div>
                                             	-->	       					            
        ...
        .then(packages => {
            ...
            if (Array.isArray(packages) && packages.length > 0) {
                packages.forEach(pkg => {
                    ...
                    option.value = pkg.id;
                    option.textContent = `${pkg.name} - ₹${pkg.price} - ${pkg.interval}`;
                    packageDropdown.appendChild(option);
                });
            } else {
                packageDropdown.innerHTML = `<option value="">No active packages found</option>`;
            }
        })
        .catch(error => {
            console.error("Failed to load packages:", error);
            packageDropdown.innerHTML = `<option value="">Unable to load packages</option>`;
        });
    });
</script>
@endpush
