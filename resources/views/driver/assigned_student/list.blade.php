...
@section('content')
    ...
    @php
        ...
        if (!$user || !$user->user_information) {
            // Skip this student if user not found or user_information missing
            continue;
        }
        ...
        if (!$info) {
            // Skip if decoding user_information failed
            continue;
        }
    ...
    <tr>
        ...
        <td class="min-w-150px">
            @php
                $vehicle = DB::table('vehicles')->where('id', $student->vehicle_id)->first();
            @endphp
            {{ $vehicle ? $vehicle->vehicle_model . ', ' . $vehicle->vehicle_number : '-' }}
        </td>
    </tr>
@endforeach
