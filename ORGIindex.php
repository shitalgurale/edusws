...
function insert_log_data($device_id, $input_valus)
{
    ...
    try {
        ...
        // Process for daily_attendances or hr_daily_attendences
        file_put_contents('face_device_debug.log', "🔍 Checking active session...\n", FILE_APPEND);
        $sql = "SELECT * FROM sessions WHERE school_id = ? AND status = 1 LIMIT 1";
        $stmt = $dbh->prepare($sql);
        $stmt->execute([$school_id]);
        $active_session = $stmt->fetch(PDO::FETCH_ASSOC);

        if (!$active_session) {
            file_put_contents('face_device_debug.log', "❌ No active session for school_id: $school_id\n", FILE_APPEND);
            return;
        } else {
            file_put_contents('face_device_debug.log', "✅ Active session found: session_id = " . $active_session['id'] . "\n", FILE_APPEND);
        }
        ...
        if ($enrollment) {
            ...
            if (!$attendance) {
                ...
                $sql = "INSERT INTO daily_attendances 
                        (school_id, section_id, student_id, class_id, device_id, stu_intime, stu_outtime, punchstatus, iomode, verifymode, session_id, created_at, updated_at) 
                        VALUES (?, ?, ?, ?, ?, ?, NULL, ?, ?, ?, ?, ?, ?)";
                $stmt = $dbh->prepare($sql);
                $stmt->execute([
                    $school_id, $enrollment['section_id'], $enrollment['user_id'], $enrollment['class_id'],
                    $device_id, $regtime, $punchStatus, $iomode, $verifymode,
                    $active_session['id'], date('Y-m-d H:i:s'), date('Y-m-d H:i:s')
                ]);
                ...
                $stmt = $dbh->prepare($sql);
                $stmt->execute([
                    $regtime, $device_id, $punchStatus, $iomode, $verifymode, date('Y-m-d H:i:s'), $attendance['id']
                ]);
