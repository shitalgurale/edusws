...
//To store the FCM Tokens for Users for posting Notifications
Route::post('/store-fcm-token', function(Request $request) {
    if (!Auth::check()) {
        Log::warning("Unauthenticated FCM token POST from IP: " . request()->ip());
    ...
    Log::info("Received FCM token for user {$user->id}: $token");
    ...
    if (!in_array($token, $existingTokens)) {
        foreach (range(1, 5) as $i) {
            ...
            if (empty($user->$field)) {
                $user->$field = $token;
                $user->save();
                Log::info("Token stored in $field for user {$user->id}");
                break;
            }
        }
    } else {
        Log::info("Duplicate token for user {$user->id}, skipping.");
    }

    return response()->json(['status' => 'success']);
})->middleware('auth');

//To broadcast notification to all users with all FCM tokens in Admin Panel
    Route::post('/admin/broadcast-notification', [AdminController::class, 'broadcastNotification'])->name('admin.broadcast.notification')->middleware('auth');

//To broadcast notification to all users with all FCM tokens in Teacher Panel
    Route::post('/teacher/broadcast-notification', [TeacherController::class, 'broadcastNotification'])->name('teacher.broadcast.notification');
    
Route::get('/teacher/marks/list/pdf', [TeacherController::class, 'marksPdf'])->name('teacher.marks.list_pdf');
...
Route::get('/parent/fetch-present-days', [ParentController::class, 'fetchPresentDays'])->name('parent.fetch_present_days');
Route::post('/parent/fetch-due-amount', [ParentController::class, 'fetchDueAmount'])->name('parent.fetch_due_amount');

Route::get('/admin/income-report', [AdminController::class, 'getIncomeReport']);
Route::get('/admin/class-wise-sections/{id}', [AdminController::class, 'classWiseSections'])->name('admin.class_wise_sections');
Route::get('/student/by-class-section', [AdmiController::class, 'getStudentsByClassSection'])->name('student.by.class.section');
Route::get('/student/by-class-section/{class_id}/{section_id}', [AdminController::class, 'getStudentsByClassAndSection'])->name('student.by.class.section');



//Route::get('/get-active-packages', [SuperadminController::class, 'getActivePackages'])->name('superadmin.packages.active');
...
//SuperadminController routes end here

//SMSCntroller routes starts here
Route::post('/send-sms', [SmsController::class, 'sendSms'])->name('sms.send');
//SMSCntroller routes end here

//AttendanceController routes starts here

    //Route::get('/attendance/student-attendance-report', [AttendanceController::class, 'studentAttendanceReport'])->name('attendance.student_attendance_report');

    Route::post('admin/attendance/student_daily_attendance', [AttendanceController::class, 'getDailyStudentAttendance'])->name('admin.attendance.student_daily_attendance');
    Route::get('admin/attendance/student_daily_attendance', [AttendanceController::class, 'studentDailyAttendancePage'])->name('admin.attendance.student_daily_attendance');
    
    Route::post('/admin/attendance/export-student-daily-pdf', [AttendanceController::class, 'exportStudentDailyPDF'])
        ->name('admin.attendance.exportStudentDailyPDF');
        
    Route::get('/attendance/employee/export-csv', [AttendanceController::class, 'exportEmployeeAttendanceCSV'])->name('attendance.employee.export_csv');
    
    
    
    Route::get('admin/attendance/hr_daily_attendance', [AttendanceController::class, 'dailyAttendancePage'])->name('admin.attendance.hr_daily_attendance');

    Route::get('admin/attendance/hr_daily_attendance/filter', [AttendanceController::class, 'filterHrDailyAttendance'])->name('admin.attendance.hr_daily_attendance.filter');

    Route::get('admin/attendance/hr_daily_attendance/export-daily-pdf', [AttendanceController::class, 'exportEmployeeDailyAttendancePDF'])->name('admin.attendance.export_daily_pdf');
    
    Route::get('teacher/export-student-csv', [AttendanceController::class, 'exportStudentAttendanceCSVTeacher'])->name('teacher.export_student_csv');


//AttendanceController routes end here

    
    //Report Card
    Route::get('student/report-card/{exam_category_id}', [StudentController::class, 'generateReportCard'])->name('student.download_report_card');
    
    // Student inbox
    Route::get('student/inbox', [App\Http\Controllers\StudentController::class, 'inbox'])->name('student.inbox');

    // Parent inbox
    Route::get('parent/inbox', [App\Http\Controllers\ParentController::class, 'inbox'])->name('parent.inbox');

    // For parents
    Route::get('/parent/messages/download/{id}', [App\Http\Controllers\ParentController::class, 'downloadAttachment'])->name('parent.messages.download');

    // For students
    Route::get('/student/messages/download/{id}', [App\Http\Controllers\StudentController::class, 'downloadAttachment'])->name('student.messages.download');

    Route::get('student/messages/{id}/preview', [StudentController::class, 'inlinePreview'])
    ->name('student.messages.preview');
    //for parent
    Route::get('parent/messages/{id}/preview', [ParentController::class, 'inlinePreview'])
    ->name('parent.messages.preview');

    //Outbox
    Route::get('admin/message/{id}/details', [AdminController::class, 'messageDetails'])
    ->name('admin.message.details');

    // Inbox
Route::get('student/message/{id}/details', [StudentController::class, 'messageDetails'])->name('student.message.details');
Route::get('parent/message/{id}/details', [ParentController::class, 'messageDetails'])->name('parent.message.details');

// Admin Route to fetch parents based on selected class
Route::get('/admin/getParentsByClass/{class_id}', [AdminController::class, 'getParentsByClass']);


Route::get('/admin/getParentsByClassAndSection/{class_id}/{section_id}', [AdminController::class, 'getParentsByClassAndSection']);
Route::get('/admin/getSectionsByClass/{id}', [AdminController::class, 'getSectionsByClass']);
Route::get('/admin/getAllSectionsBySchool', [AdminController::class, 'getAllSectionsBySchool']);
Route::get('/admin/getAllStudentsBySchool', [AdminController::class, 'getAllStudentsBySchool']);
Route::get('/admin/getAllParentsBySchool', [AdminController::class, 'getAllParentsBySchool']);
Route::get('/admin/getAllTeachersBySchool', [AdminController::class, 'getAllTeachersBySchool']);
Route::get('/admin/getTeachersByClassAndSection/{class_id}/{section_id}', [AdminController::class, 'getTeachersByClassAndSection']);


Route::post('/admin/parent/update/{id}', [AdminController::class, 'parentUpdate'])->name('admin.parent.update');

// ✅ Teacher routes for fetching data (same as Admin)


Route::get('admin/student/filter', [AdminController::class, 'filterStudents'])->name('admin.student.filter');


// Get parents by class
Route::get('/teacher/getParentsByClass/{class_id}', [TeacherController::class, 'getParentsByClass']);

// Get Section by class
Route::get('/admin/getStudentsByClassAndSection/{class_id}/{section_id}', [AdminController::class, 'getStudentsByClassAndSection']);

// Get students by class and section
Route::get('/teacher/getStudentsByClassAndSection/{class_id}/{section_id}', [TeacherController::class, 'getStudentsByClassAndSection']);

// Get parents by class and section
Route::get('/teacher/getParentsByClassAndSection/{class_id}/{section_id}', [TeacherController::class, 'getParentsByClassAndSection']);

// Get sections by class
Route::get('/teacher/getSectionsByClass/{id}', [TeacherController::class, 'getSectionsByClass']);

// Get all sections in the school
Route::get('/teacher/getAllSectionsBySchool', [TeacherController::class, 'getAllSectionsBySchool']);

// Get all students in the school
Route::get('/teacher/getAllStudentsBySchool', [TeacherController::class, 'getAllStudentsBySchool']);

// Get all parents in the school
Route::get('/teacher/getAllParentsBySchool', [TeacherController::class, 'getAllParentsBySchool']);

Route::get('/admin/getTeachersByClassAndSection/{class_id}/{section_id}', [AdminController::class, 'getTeachersByClassAndSection']);
Route::get('teacher/compose/inbox', [TeacherController::class, 'inbox'])->name('teacher.inbox');
Route::get('teacher/compose/details/{id}', [TeacherController::class, 'messageDetails'])->name('teacher.message.details');
//Route::get('teacher/messages/preview/{id}', [TeacherController::class, 'inlinePreview'])->name('teacher.messages.preview');
//Route::get('teacher/messages/download/{id}', [TeacherController::class, 'downloadAttachment'])->name('teacher.messages.download');
...
Route::controller(AdminController::class)->middleware('admin','auth')->group(function () {
    ...
    Route::get('/admin/compose', 'adminCompose')->name('admin.compose');
    Route::post('/admin/send-mail', 'sendMail')->name('admin.send_mail');
    Route::get('/admin/outbox', 'adminOutbox')->name('admin.outbox');
    Route::get('messages/download/{id}', 'downloadAttachment')->name('admin.messages.download');
    Route::get('messages/preview/{id}', 'inlinePreview')->name('admin.messages.preview');
    Route::get('admin/messages/preview/{id}', 'inlinePreview')->name('admin.messages.preview');
    Route::get('admin/messages/download/{id}', 'downloadAttachment')->name('admin.messages.download');
    ...
    Route::post('admin/daily-attendance/export-csv', 'dailyAttendanceFilter_csv')->name('admin.dailyAttendanceFilter_csv');
    ...
    Route::post('/admin/marks/upload', 'uploadMarks')->name('admin.marks.upload');
 //   Route::get('/admin/marks/csv-preview', function () {return view('admin.marks.partials.marks_csv_preview');})->name('admin.marks.csv_preview');
    Route::get('/admin/marks/csv-preview', function () {return view('admin.marks.csv_preview');})->name('admin.marks.csv_preview');
    Route::post('admin/marks/upload', 'uploadMarks')->name('admin.marks.upload');
 //   Route::get('/admin/marks/csv-preview', 'csvPreview')->name('admin.marks.csv_preview');
    ...
    //Certificate
    Route::get('/admin/certificate/getSectionsByClass/{class_id}', [AdminController::class, 'getSectionsByClass']);
    Route::get('/admin/certificate/getStudentsByClassAndSection/{class_id}/{section_id}', [AdminController::class, 'getStudentsByClassAndSection']);

    Route::post('admin/certificate/generateBonafideUI', [AdminController::class, 'generateBonafideUI'])->name('admin.certificate.generateBonafideUI');

    Route::prefix('admin/certificate')->group(function () {
    Route::get('/', [AdminController::class, 'showCertificateUI'])->name('admin.certificate.index');
    Route::post('/generate-bonafide', [AdminController::class, 'generateBonafideUI'])->name('admin.certificate.bonafide');
    Route::get('/view-bonafide/{id}', [AdminController::class, 'viewBonafide'])->name('admin.certificate.viewBonafide');
    Route::get('/download-bonafide/{id}', [AdminController::class, 'downloadBonafide'])->name('admin.certificate.downloadBonafide');

    Route::post('/generate-tc', [AdminController::class, 'generateTCUI'])->name('admin.certificate.tc');
    Route::get('/view-tc/{id}', [AdminController::class, 'viewTC'])->name('admin.certificate.viewTC');
    Route::get('/download-tc/{id}', [AdminController::class, 'downloadTC'])->name('admin.certificate.downloadTC');
});
...
//ATTENDANCE CONTROLLER ROUTES START HERE

    Route::post('admin/export-student-daily-csv', [AttendanceController::class, 'exportStudentDailyCSV'])
    ->name('admin.exportStudentDailyCSV');
    
    Route::post('admin/attendance/hr-daily-export-csv', [AttendanceController::class, 'exportHrDailyCSV'])->name('admin.exportHrDailyCSV');
...
Route::controller(TeacherController::class)->middleware('teacher','auth')->group(function () {
    ...
    Route::get('/teacher/compose', 'teacherCompose')->name('teacher.compose');
    Route::post('/teacher/send-mail', 'sendMail')->name('teacher.send_mail');
    Route::get('/teacher/outbox', 'teacherOutbox')->name('teacher.outbox');
    Route::get('teacher/messages/download/{id}', 'downloadAttachment')->name('teacher.messages.download');
    Route::get('teacher/messages/preview/{id}', 'inlinePreview')->name('teacher.messages.preview');
    Route::get('teacher/messages/preview/{id}', 'inlinePreview')->name('teacher.messages.preview');
    Route::get('teacher/messages/download/{id}', 'downloadAttachment')->name('teacher.messages.download');
    Route::get('/teacher/getParentsByClass/{classId}','getParentsByClass');
    ...
    Route::get('/admin/take_attendance_modal', [AdminController::class, 'takeAttendance'])->name('admin.take_attendance_modal');
    Route::get('/admin/get-sections', [CommonController::class, 'getSections'])->name('admin.get_sections');
...
//Route::get('student_fee/invoice/{id}', [AccountantController::class, 'studentFeeinvoice'])->name('accountant.studentFeeinvoice');
//Accountant routes are here
    ...
    Route::get('student_fee/invoice/{id}', 'studentFeeinvoice')->name('accountant.studentFeeinvoice');
...
// AttendanceController - Shital

// Route::any('/', function (Request $request) {
//     file_put_contents(storage_path('logs/face_device_test.log'), date('Y-m-d H:i:s') . " - HIT\n", FILE_APPEND);
//     file_put_contents(storage_path('logs/face_device_test.log'), "HEADERS:\n" . print_r($request->headers->all(), true), FILE_APPEND);
//     file_put_contents(storage_path('logs/face_device_test.log'), "BODY:\n" . $request->getContent() . "\n\n", FILE_APPEND);
//     return response("OK RAW", 200);
// });


Route::get('/attendance', [AttendanceController::class, 'getAttendance']);
Route::get('/view-attendance', [AttendanceController::class, 'showAttendance'])->name('attendance.index');
Route::post('/postattendance', [AttendanceController::class, 'postAttendance']);
//Route::get('/attendance/students', [AttendanceController::class, 'showStudentAttendance']);
//Route::get('/attendance/employees', [AttendanceController::class, 'showEmployeeAttendance']);


Route::post('/teacher/update-student-attendance-data', [AttendanceController::class, 'updateStudentAttendanceData'])
    ->name('teacher.update_student_attendance_data');
Route::get('/teacher/export-student-attendance-pdf', [AttendanceController::class, 'exportStudentAttendancePDF'])
    ->name('teacher.export_student_pdf');


Route::post('/attendance/update-student-attendance-data', [AttendanceController::class, 'updateStudentAttendanceData'])
    ->name('attendance.update_student_attendance_data');
Route::get('/attendance/student/export-pdf', [AttendanceController::class, 'exportStudentAttendancePDF'])
    ->name('attendance.export_student_pdf');
Route::get('/admin/daily-attendance-filter', [AdminController::class, 'dailyAttendanceFilter'])
    ->name('admin.dailyAttendanceFilter');


Route::post('/attendance/update-employee-attendance-data', [AttendanceController::class, 'updateEmployeeAttendanceData'])
    ->name('attendance.update_employee_attendance_data');
Route::get('/attendance/employee/export-pdf', [AttendanceController::class, 'exportEmployeeAttendancePDF'])
    ->name('attendance.export_employee_pdf');


Route::get('/attendance/employee/view-report', [AttendanceController::class, 'viewEmployeeAttendanceReport'])
    ->name('attendance.view_employee_report');

//Route::get('/admin/download-report-card/{student_id}', [AdminController::class, 'generateReportCard'])
 //   ->name('admin.download_report_card');

Route::get('/admin/report-card/{student_id}/{exam_category_id}', [AdminController::class, 'generateReportCard'])
    ->name('admin.download_report_card');

Route::get('/parent/download-report-card/{student_id}/{exam_category_id}', [ParentController::class, 'generateReportCard'])
    ->name('parent.download_report_card');
    
Route::get('/teacher/download-report-card/{student_id}/{exam_category_id}', [TeacherController::class, 'downloadReportCard'])->name('teacher.download_report_card');



//Face Device Attendance Route
//Route::post('/', [FaceController::class, 'receiveAttendance']);

// //Face Device Attendance Route
 Route::post('/', [FaceController::class, 'faceattendance']);

// Route::post('/', function () {
//     \Log::info('✅ Laravel is handling the POST / route');
//     return response('Laravel handled the request', 200);
// });

// Face Device Attendance Route Inline for Testing
// Route::post('/', function () {
//     \Log::info('✅ Laravel is handling the POST / route');
