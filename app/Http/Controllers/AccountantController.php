     ...
     public function studentFeeManagerList(Request $request)
{
    ...
    \Log::info("[Accountant] 📥 studentFeeManagerList called by User ID: " . auth()->id());

    if (count($request->all()) > 0) {
        \Log::info("[Accountant] 🔍 Request parameters:", $request->all());
        ...
        \Log::info("[Accountant] 🗓️ Date Range: From " . date('d-M-Y', $date_from) . " To " . date('d-M-Y', $date_to));
        \Log::info("[Accountant] 🏷️ Class Filter: $selected_class | Status Filter: $selected_status");
        ...
        if ($selected_class != "all") {
            ...
            \Log::info("[Accountant] ✅ Class ID filter applied: $selected_class");
        }
        ...
        if ($selected_status != "all") {
            if ($selected_status == "partial") {
                $query->whereColumn('paid_amount', '<', 'total_amount')
                      ->where('paid_amount', '>', 0);
                \Log::info("[Accountant] ✅ Partially Paid filter applied");
            } elseif ($selected_status == "paid") {
                $query->whereColumn('paid_amount', '=', 'total_amount');
                \Log::info("[Accountant] ✅ Paid filter applied");
            } elseif ($selected_status == "unpaid") {
                $query->where(function ($q) {
                    $q->whereNull('paid_amount')->orWhere('paid_amount', '=', 0);
                });
                \Log::info("[Accountant] ✅ Unpaid filter applied");
            } else {
                \Log::warning("[Accountant] ⚠️ Unknown status: $selected_status");
            }
        }

        $invoices = $query->get();
        \Log::info("[Accountant] 📦 Invoices retrieved: " . $invoices->count());

        $classes = Classes::where('school_id', $school_id)->get();

        return view('accountant.student_fee_manager.student_fee_manager', [
            'classes' => $classes,
            'invoices' => $invoices,
            'date_from' => $date_from,
            'date_to' => $date_to,
        ...
        \Log::info("[Accountant] 🆕 No filters applied, loading current month");
        ...
        \Log::info("[Accountant] 📦 Default invoices retrieved: " . $invoices->count());
...
{
    \Log::info("[Accountant] 📥 studentFeeinvoice() called with Invoice ID: $id");

    $invoice = StudentFeeManager::find($id);
    if (!$invoice) {
        \Log::error("[Accountant] ❌ Invoice not found for ID: $id");
    ...
    $student_details = (new CommonController)->get_student_details_by_id($student_id);
    ...
    $school_address = $school->address ?? 'Not Available';
    $school_phone   = $school->phone ?? 'Not Available';
    $school_email   = $school->email ?? 'Not Available';

    $session_id = get_school_settings($school_id)->value('running_session');
    $session = \App\Models\Session::find($session_id);
    $session_title = $session ? $session->session_title : 'Unknown';

    $installments = \App\Models\FeeInstallment::where('invoice_id', $invoice->id)->orderBy('paid_at')->get();

    return view('accountant.student_fee_manager.invoice', [
