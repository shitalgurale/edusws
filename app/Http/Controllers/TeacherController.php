    ...
    public function marksFilter(Request $request)
{
    ...
    $marks_data['class_name']    = $class?->name ?? 'N/A';
    $marks_data['section_name']  = $section?->name ?? 'N/A';
    $marks_data['subject_name']  = $subject?->name ?? 'N/A';
    $marks_data['session_title'] = $session?->session_title ?? 'N/A';

    $enroll_students = \App\Models\Enrollment::where('class_id', $data['class_id'])
        ->where('section_id', $data['section_id'])
        ->get();

    $view = view('teacher.marks.marks_list', [
    ...
    public function marks($value = '')
{
    ...
    Log::info('✅ Fetched exam categories, classes, sessions.', [
        'exam_categories' => $page_data['exam_categories']->count(),
        'classes'         => $page_data['classes']->count(),
        'sessions'        => $page_data['sessions']->count()
    ]);

    return view('teacher.marks.index', $page_data);
}

  
public function marksFilter(Request $request)
{
    Log::info('🎯 TeacherController@marksFilter started.', ['request_data' => $request->all()]);
    ...
    Log::info('📌 Filters assigned to $marks_data', $marks_data);
    ...
    Log::info('👨‍🎓 Enrolled students fetched', ['count' => $enroll_students->count()]);
    ...
    if ($exam) {
        ...
        Log::warning('⚠️ No exam found. Please create exam first.', ['filters' => $data]);

        return response()->json([
            'status' => 'error',
            'message' => 'No records found. Please create exam for the selected filter.'
        ]);
    }
}
...
{
    \Log::info("➡️ TeacherController@marksPdf started", $request->all());
    ...
    $enroll_students = \App\Models\Enrollment::where('class_id', $class_id)
    ...
    \Log::info("📘 Class/Section/Subject/Session fetched", [
        'class' => $class?->name,
    ...
    $marks_data = [
        ...
        'session_title' => $session?->session_title ?? 'N/A',
    ...
    \Log::info("📄 PDF view data prepared", $marks_data);
    ...
    $pdf = \PDF::loadView('teacher.marks.markPdf', $data);

    \Log::info("✅ PDF loaded, ready to download");
...
{
    \Log::info('dailyAttendanceFilter_csv started', [
        'month' => $request->input('month'),
        'year' => $request->input('year'),
        'role_id' => $request->input('role_id'),
    ]);

    // Parse key-based form submission data
    $data = $request->all();
    ...
    $first_date_str = '01 ' . $data['month'] . ' ' . $data['year'];
    $first_date = strtotime($first_date_str);
    $last_date = strtotime(date("Y-m-t", $first_date));

    // For datetime column comparison
    $first_date_db = date('Y-m-d 00:00:00', $first_date);
    $last_date_db = date('Y-m-d 23:59:59', $last_date);
    ...
    // Fetch attendance records
    $attendance_of_students = DailyAttendances::whereBetween('timestamp', [$first_date_db, $last_date_db])
        ->where([
            'school_id' => $school_id,
            'session_id' => $active_session
        ])
    ...
    \Log::info('✅ Records fetched', ['count' => $attendance_of_students->count()]);

    if ($attendance_of_students->isEmpty()) {
        return response()->json(['message' => 'No attendance records found for selected filters.'], 404);
    }

    $attendance_array = $attendance_of_students->toArray();
    $student_ids = array_unique(array_column($attendance_array, 'student_id'));

    // CSV Header
    $csv_content = "Student/Date";
    ...
    foreach ($student_ids as $student_id) {
        ...
        $user_details = (new CommonController)->get_user_by_id_from_user_table($student_id);
        $csv_content .= $user_details['name'] . ',';

        for ($i = 1; $i <= $days_in_month; $i++) {
            $date_string = sprintf("%02d %s %s", $i, $page_data['month'], $page_data['year']);
            $timestamp = strtotime($date_string);
            $date_db = date('Y-m-d 00:00:00', $timestamp);

            $attendance = DailyAttendances::where([
                'student_id' => $student_id,
                'school_id' => $school_id,
                'session_id' => $active_session,
                'timestamp' => $date_db
            ...
            if ($attendance) {
                $csv_content .= $attendance->status == 1 ? "P," : "A,";
            } else {
                $csv_content .= ",";
            }

            if ($i == $days_in_month) {
                $csv_content = substr_replace($csv_content, "", -1); // Remove trailing comma
            }
        }

        \Log::info("👤 Processed student", ['student_id' => $student_id, 'name' => $user_details['name']]);
    }

    // Generate and return CSV as response
    $filename = 'Attendance_Report_' . $data['month'] . '_' . $data['year'] . '.csv';
    $temp_path = storage_path($filename);
    file_put_contents($temp_path, $csv_content);

    \Log::info("📁 CSV file created", ['path' => $temp_path]);

    return response()->download($temp_path)->deleteFileAfterSend(true);
}
