...
public function sendSms(Request $request)
{
    ...
    $this->msg91($mobiles, $studentName, $dateTime, $workedHours);
...
*/
 ...
 /**
    ...
    /**
     * Public API to trigger sending SMS via frontend (JS / form)
     */
    public function sendSms(Request $request)
    {
        $request->validate([
        ...
        Log::info("🚀 SMS Requested → Mobiles: $mobiles, Name: $studentName, Time: $dateTime, Hours: $workedHours");
...
public function receivers(Request $request){
    return view('admin.sms_center.receivers', [
...
public function receivers(Request $request){
    ...
    $teachers = [];
    if ($receiver === 'teacher' && $class_id && $section_id) {
        $teacher_ids = TeacherPermission::where([
            'class_id' => $class_id,
   ...
   public function smsSending(Request $request)
{
    ...
    if (!empty($phones) && !empty($messages)) {
        foreach ($phones as $key => $phone) {
            ...
            if ($sms_settings[0]->active_sms == 'none') {
                return redirect()->back()->with('error', 'Please activate an SMS gateway.');
            }

            if ($sms_settings[0]->active_sms == 'msg91') {
                // Prepare 4 required values
                $studentName = 'User'; // You can customize this or pass from request
                $dateTime = now()->format('Y-m-d H:i:s');
                $workedHours = '00:00'; // Customize as per logic if needed

                // ✅ Pass all 4 arguments to msg91
                $this->msg91($phone, $studentName, $dateTime, $workedHours);
            }
        ...
        return redirect()->back()->with('error', 'Please select at least one number and ensure messages are not empty.');
    }
}
