  ...
  public function roleWiseUserlist(Request $request)
{
    ...
    try {
        // ✅ Step 1: Fetch users for the selected HR role
        $users = Hr_user_list::where([
            'hr_roles_role_id' => $data['role_id'],
            'school_id' => $user->school_id
        ])->get();
        ...
        \Log::info("Users fetched count: $userCount");
        if ($userCount > 0) {
            \Log::info('Sample user data:', [$users->first()->toArray()]);
        }

        // ✅ Step 2: Fetch existing attendance for selected date and role
        $attendance_date = date('Y-m-d', strtotime($data['date']));

        $existing_attendance = HrDailyAttendence::whereDate('emp_intime', $attendance_date)
            ->where('hr_roles_role_id', $data['role_id'])
            ->where('school_id', $user->school_id)
            ->get()
            ->keyBy('user_id'); // So we can access like $existing_attendance[user_id]

        \Log::info("Existing attendance count for date $attendance_date: " . $existing_attendance->count());

        // ✅ Step 3: Render the view with user list and existing attendance
        return view('hr_daily_attendence.attendence_view', [
            'users' => $users->toArray(),
            'date' => $data['date'],
            'role_id' => $data['role_id'],
        ...
        \Log::error('Error in roleWiseUserlist(): ' . $e->getMessage());
...
public function hrdailyAttendanceFilter_csv(Request $request)
{
    ...
    $roleName = ($role_id !== 'All') 
        ? (Hr_roles::find($role_id)->name ?? 'Role') 
        : 'All_Roles';
    ...
    $callback = function () use ($attendances, $columns) {
        ...
        foreach ($attendances as $row) {
            // Log to verify each row
            \Log::info('📤 Exporting row', [
                'user' => $row->user->name ?? 'N/A',
                'role' => $row->role->name ?? 'N/A',
